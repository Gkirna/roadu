...
export default function Achievements() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [achRes, earnedRes, profileRes] = await Promise.all([
      ...
      return {
        ...
        earnedIds: new Set((earnedRes.data || []).map((e) => e.achievement_id)),
        userXp: profileRes.data?.xp ?? 0,
      };
    },
    enabled: !!user,
    staleTime: 30_000,
  ...
  const { achievements, earnedIds, userXp } = data;
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">{earnedCount} of {achievements.length} unlocked</p>
      </div>
        ...
        <Card className="border-primary/20 bg-primary/5">
          <CardContent className="p-5 space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-foreground whitespace-nowrap">{userXp} / {nextAchievement.xp_threshold} XP</span>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid sm:grid-cols-2 gap-4">
        {achievements.map((ach) => {
          ...
          const progressToThis = ach.xp_threshold ? Math.min(100, Math.round((userXp / ach.xp_threshold) * 100)) : 0;
          return (
            <motion.div key={ach.id} variants={item}>
              <Card className={`transition-all ${earned ? "border-secondary/30 bg-secondary/5" : "border-border/50 opacity-70"}`}>
                <CardContent className="p-4 flex items-start gap-3">
                  <div className={`h-12 w-12 rounded-xl flex items-center justify-center text-2xl shrink-0 ${earned ? "bg-secondary/15" : "bg-muted"}`}>
                    {earned ? ach.icon || "🏆" : <Lock className="h-5 w-5 text-muted-foreground" />}
                  </div>
                  <div className="flex-1 min-w-0 space-y-1.5">
                    <div className="flex items-center gap-2">
                      <h3 className={`font-semibold text-sm ${earned ? "text-foreground" : "text-muted-foreground"}`}>{ach.title}</h3>
                    ...
                    {earned && <span className="text-[10px] text-secondary font-medium">✓ Unlocked</span>}
                  </div>
                </CardContent>
              </Card>
            </motion.div>
